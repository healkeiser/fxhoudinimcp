# Working discipline for this server (the full rules; the server instructions are the summary)
...
2.  NEVER GUESS, LOOK IT UP. `get_node_card(node_type, context)` returns real connector labels, real parameter names/defaults/menus and Houdini's own help text for THIS version. `search_help(query)` + `get_help_page(path)` serve EVERY corpus the install ships, which on a full 22.0 is 56 of them and around 11,000 pages. That includes SideFX's own workflow manuals, not just node reference: `pyro/lookdev`, `fluid/tips`, `vellum/vellumtips`, `destruction/constraints`, `mpm/troubleshooting`, `model/attributes`, `copy/instancing`, `assets/versioning_systems`, `solaris/about_lops`, `tops/attributes`, `render/tips`, `crowds/basics`, `character/kinefx/index`, `copernicus/spaces`, `heightfields/layers`. Read the workflow page before designing a setup, and the node page before setting a parameter. When `search_help` ranks a `shelf/...` or workflow page above the node pages, that page is the answer to "how is this normally done": read it first, and do not open a node page instead. `get_parameters(node_path, patterns=[...])` reads many live values at once, and it is the ONLY view of parameters a node grows on instantiation: Render Geometry Settings, Karma Render Properties and Edit Properties LOPs build most of their `xn__...` parameters when created, so the type card shows 16 of the 208 and the live node has the rest, and the card now reports hidden parameters and multiparm blocks: a name containing `#` is an instance template, so the real parameter is `source_volume1`, not `source_volume#`. Guessed parameter names are the #1 source of silently broken setups. Solaris edit nodes (Camera, Light, Render Settings, Edit Properties) ship with every `*_control` menu on "Set", so a node added for one attribute silently overwrites all the others on the imported prim: switch the controls you do not mean to author to "Do Nothing" (`none`). A 50 mm default lens over an imported 24 mm camera is what that mistake looks like.
3.  VERIFY, THEN CLAIM. After any build or change, `verify_network(parent)` (the middle-click-everything pass); read `error_nodes` and the geometry counts. At visual milestones `capture_screenshot` and LOOK at the image. A tool returning success is not evidence.
    A Karma viewport needs seconds to converge after any change: pass `settle_seconds` to `capture_screenshot` / `render_viewport` rather than sleeping in a shell between calls. For a frame on disk use `start_render(rop_path)` and `get_render_progress(rop_path)`: a ROP that fails (a missing license, a bad output path) says so there within seconds, where a `render()` inside `execute_python` followed by a seven-minute wait for a file says nothing.
    A QUESTION IS ANSWERED BEFORE ANYTHING IS CHANGED. "I can't see anything in the viewport" is a request for a diagnosis: find the cause (renderer, camera, display node, frame, visibility), state it, fix that one thing, and report. It is not an invitation to relight the shot, move the camera or rework the whitewater; those are new tasks the user has not asked for, and thirty minutes of them with no reply is the worst possible answer to a question.
    TWO FAILED FIXES MEANS READ, NOT A THIRD FIX. When the same symptom survives two attempts, the design is wrong, not the numbers: stop measuring, call `get_workflow_guide(topic)` for the subject, and open the top `shelf/...` or workflow hit `search_help` gave you (a shelf page describes how SideFX assembles that very setup). A session spent forty minutes and four resims hand-blending a FLIP tank into an ocean before reading `shelf/guidedoceanlayer`, which its own first search had ranked first, and which names the boundary layer that keeps the level matched.
...
4.  DRAFT FIRST, THEN UPRES. Coarse divsize, low point counts, few substeps. Present that, and raise quality only once the look is approved. Never make the user wait on a hero cook of an unapproved setup. Raising resolution or launching a full-range cache is a cost the user pays in minutes of wall-clock: say what it will cost and get a yes first, and never fold an upres into a look-fix round the user did not ask for.
5.  CACHE CHECKPOINTS. A `filecache` is mandatory as soon as a stage costs about 3 seconds or more per frame, and always when a simulation runs at draft settings that will be raised for production later (the cache is what lets the look be approved without re-cooking the sim on every readback). Measure with `cook_frame_range` or `find_expensive_nodes`; a light stage, a few hundred milliseconds a frame during R&D, is cheaper to leave live than to write to disk. Caches are the seams between stages of the shot. A plain `filecache` is not always the right node: RBD and Vellum have `rbdio`/`vellumio`, which carry every stream of the sim (constraints, proxy and collision geometry, points) where a lone `filecache` on the first output silently drops the rest; if a `filecache` is used on Vellum anyway, put `vellumpack` in front so `vellumpostprocess` can work downstream of the cache. Other solvers have their own I/O or pack nodes; `search_help("<solver> cache")` and the solver's workflow guide say which. Run a cache with `write_cache(node_path, frame_range=[a, b])`. It runs in Houdini, in the foreground, where the user sees Houdini's own progress dialog and can cancel; the call holds until the write is done, up to an hour, and returns a verdict with the files on disk. A client that hands a long call to a background task notifies you when the verdict lands: wait for it, call nothing else in Houdini meanwhile (every other command queues behind the write), and never poll the disk with a shell `sleep` or `until` loop. A verified write turns the node's Load from Disk on. `background=True` writes from a separate hython and returns at once, but the user then sees nothing in Houdini, so use it only when they ask to keep working during a write, and follow it with `get_cache_status` (`complete`, `missing_frames`, `writing`, `hint`). Pressing buttons through `execute_python` is never needed. The same holds for `start_render(node_path, frame_range)` on a ROP or a ROP chain; `get_render_progress` reports errors, files and a background process; `license_error` there or in `verify_network` means no scene change will help, tell the user. A "Save to Disk" or `render()` pressed with `execute_python` blocks Houdini for the whole write, times out the client, and leaves every later command queued behind it: a live session did this to two 80-frame FLIP caches and spent the next forty minutes polling the disk from a shell. When a call does time out, the answer is background mode, not a longer timeout. While a background job runs, keep working on something else and check on it between steps. Save the hip first, since the default cache path is built from `$HIP`/`$HIPNAME`, and prefer `bgeo.sc`.
...
7.  ISOLATE WHILE ITERATING. The viewport cooks every displayed object at every frame change, so a FLIP sim left visible makes iterating on a light rain setup slower by orders of magnitude. When the user is working on one element, turn the display flag off on every other heavy object (`set_node_flags(node_path, display=False)` on the `/obj` container) and leave it off until the work moves on. Do not blindly re-enable everything for a check: verify and capture the element under work with only what it needs visible, and turn the rest back on only for a full-scene review the user asked for.
8.  KEEP IT LIGHT. copytopoints with Pack and Instance enabled packs the source once and shares it between copies, instead of duplicating it per copy. For copies needing no target points, `duplicate` is the node. At very large counts stop making real geometry: render-time instancing or packed/Alembic primitives. When something is slow, `find_expensive_nodes(root)`, profile rather than guess.
...
3.  Native nodes via `create_node` / `create_lop_node` / `create_cop_node` / `create_chop_node` + `connect_nodes_batch`, for one-or-two-node edits to existing networks. `set_parameters` (batch) sets multiple params in one call. To splice a node into an existing chain: `build_network` with the upstream node as its input, then one `connect_nodes_batch` that points the downstream node at it. VOP inputs are addressed by name (`input_name="base_color"`), never by a guessed index. `write_cache(node_path, frame_range=[a, b], background=True)` starts a File Cache without blocking the session; pressing buttons through `execute_python` is never needed for a cache.
4.  VEX wrangles via `create_wrangle`, or `attribwrangle`/`python` inside a `build_network` spec — ONLY when no COMBINATION of built-in nodes expresses the logic, and only after `list_node_types`. "No single node does this" is not a reason: Houdini is nodal and nodes are meant to be chained. Three points are an `add`; a per-piece measurement is `connectivity` + `measure` + `attribpromote`; a classification by size is `attribpromote` + `group` by expression or `partition`; moving geometry onto a surface is `ray`; a rigid fit is `extracttransform`. A wrangle or Python SOP where two or three stock nodes would do is the mark of someone who does not know the nodes, and it hides the logic from the user in a text field. When a dry-run rejects a native node's parameter, look the name up on the card; do not switch to VEX.
...
Some setups are not built from nodes at all. `list_shelf_tools(filter)` finds them, `get_shelf_tool_script(name)` shows SideFX's own recipe and the toolutils module it calls, and `run_shelf_tool(name)` runs it. The ocean procedural's internals come from a shelf tool, so `build_network` cannot produce them and guessing at them wastes a session. Tools that ask for a viewport selection (the FLIP ocean layer, collide-with, most "select the object, then" tools) are refused by `run_shelf_tool`, because through the bridge they would freeze Houdini waiting for a click: read their script and build the same nodes with `build_network`, pointing them at the geometry the tool would have asked for. `build_network` specs also take `expressions: {{parm: "..."}}` per node, so a rig whose values come from another node needs no Python; and `get_geometry_info(node_path, output_index=1)` reads a node's second and third outputs.
...
## WORKFLOW GUIDES — `get_workflow_guide(topic)` before designing a setup you have not built before
...
call `get_workflow_guide(topic)` rather than reasoning from first principles.
The same guides are also MCP prompts, which some clients expose to the user as
slash commands; the tool is the way YOU fetch one.
