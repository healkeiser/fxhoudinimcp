...
class TestDops:
    ...
    def dopnet(self, call) -> str:
        geo = call("nodes.create_node", parent_path="/obj", node_type="dopnet", name="sim")[
            "node_path"
        ]
...
class TestCops:
    def test_cop_create_and_inspect(self, call):
        copnet = call("nodes.create_node", parent_path="/obj", node_type="copnet", name="comp")[
            "node_path"
        ]
...
class TestChops:
    def test_chop_data_and_export(self, call):
        chopnet = call("nodes.create_node", parent_path="/obj", node_type="chopnet", name="motion")[
            "node_path"
        ]
        wave = call("chops.create_chop_node", parent_path=chopnet, chop_type="wave")
        ...
        geo = call("nodes.create_node", parent_path="/obj", node_type="geo", name="target")[
            "node_path"
        ]
        call(
...
class TestHda:
    def test_hda_create_inspect_roundtrip(self, call, tmp_path):
        geo = call("nodes.create_node", parent_path="/obj", node_type="subnet", name="asset")[
            "node_path"
        ]
        ...
        if sections["status"] == "success":
            ...
            name = (
                ...
                else (listed[0] if listed else "Contents")
            )
            call(
...
class TestCache:
    def test_cache_status_and_listing(self, call, tmp_path):
        geo = call("nodes.create_node", parent_path="/obj", node_type="geo", name="cached")[
            "node_path"
        ]
        box = call("nodes.create_node", parent_path=geo, node_type="box")["node_path"]
        fc = call("nodes.create_node", parent_path=geo, node_type="filecache", name="cache1")[
            "node_path"
        ]
...
class TestCode:
    def test_hscript_and_python(self, call):
        ...
        result = call("code.evaluate_expression", expression="1 + 1", language="python")
...
class TestContext:
    def test_context_introspection(self, call):
        ...
        box = call("nodes.create_node", parent_path=geo, node_type="box")["node_path"]
        xform = call("nodes.create_node", parent_path=geo, node_type="xform")["node_path"]
...
class TestRendering:
    def test_geometry_rop_renders_a_real_file(self, call, tmp_path):
        geo = call("nodes.create_node", parent_path="/obj", node_type="geo", name="g")["node_path"]
        box = call("nodes.create_node", parent_path=geo, node_type="box")["node_path"]
        rop = call("nodes.create_node", parent_path="/out", node_type="geometry", name="bake")[
            "node_path"
        ]
...
class TestRemainingNodesParamsScene:
    def test_move_reorder_color(self, call):
        geo_a = call("nodes.create_node", parent_path="/obj", node_type="geo", name="a")[
            "node_path"
        ]
        geo_b = call("nodes.create_node", parent_path="/obj", node_type="geo", name="b")[
            "node_path"
        ]
        ...
        merge = call("nodes.create_node", parent_path=geo_b, node_type="merge")["node_path"]
        s1 = call("nodes.create_node", parent_path=geo_b, node_type="sphere")["node_path"]
        call(
...
class TestLopsModule:
    ...
    def test_usd_lighting(self, call):
        lopnet = call("nodes.create_node", parent_path="/obj", node_type="lopnet", name="lights")[
            "node_path"
        ]
        light = call("lops.create_light", parent_path=lopnet, light_type="dome", intensity=2.0)
...
class TestMopUp:
    ...
    def test_geometry_inspection_extras(self, call):
        geo = call("nodes.create_node", parent_path="/obj", node_type="geo", name="g")["node_path"]
        box = call("nodes.create_node", parent_path=geo, node_type="box")["node_path"]
        grp = call("nodes.create_node", parent_path=geo, node_type="groupcreate", name="top_grp")[
            "node_path"
        ]
        call("nodes.connect_nodes", source_path=box, dest_path=grp)
        values = call("geometry.get_attrib_values", node_path=box, attrib_name="P", count=8)
        assert values["total_elements"] == 8
        assert len(values["values"]) == 8 * 3
        info = call("geometry.get_attribute_info", node_path=box, attrib_name="P")
        assert info, info
    ...
    def test_context_and_scene_extras(self, call):
        ...
        assert ctx, ctx
        schema = call("parameters.get_parameter_schema", node_path=geo, filter="translate")
    ...
    def test_vex_extras(self, call):
        ...
        wrangle = call("vex.create_wrangle", parent_path="/obj/g", vex_code="@Cd = {1,1,1};")[
            "node_path"
        ]
    ...
    def test_lops_and_materials_extras(self, call):
        ...
        sphere = call("lops.create_lop_node", parent_path=lopnet, lop_type="sphere")["node_path"]
    ...
    def test_rendering_and_viewport_extras(self, call, tmp_path):
        ...
        cam = call("nodes.create_node", parent_path="/obj", node_type="cam", name="cam1")[
            "node_path"
        ]
    ...
    def test_flip_and_vellum_workflows_build_real_networks(self, call):
        geo = call("nodes.create_node", parent_path="/obj", node_type="geo", name="src")[
            "node_path"
        ]
        ...
        cloth_geo = call("nodes.create_node", parent_path="/obj", node_type="geo", name="cloth")[
            "node_path"
        ]
